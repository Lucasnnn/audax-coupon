...
**Consequences:** migrations e SQL do adapter são revisáveis por DBA; versões do Drizzle devem estar patchadas; identificadores SQL dinâmicos só com allowlist; listagem in-memory e Postgres devem ordenar de forma estável (por código) para paginação previsível.
