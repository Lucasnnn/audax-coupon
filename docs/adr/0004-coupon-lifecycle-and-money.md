...
Valores monetários (`FIXED`, `Minimum order amount`) usam inteiros em centavos — `Discount value` em `FIXED` é estritamente positivo. Percentual usa inteiro 1–100. Para `FIXED`, `Minimum order amount` é obrigatório e deve ser ≥ ao valor do desconto. O teto alinhado a `INTEGER` (e o motivo de não usar bigint agora) está em [ADR 0007](0007-money-integer-ceiling.md).
